/* *设计思路 * 元素必须挂载到body上，这样才不会被其他元素的overflw：hidden给遮挡住 */
<template>
  <div class="relative" ref="popoverref" @focus="getfocus" @blur="losefocus" v-click-away="closepop">
    <!--    冒泡插槽-->
    <div v-if="visible" ref="contentref" class="absolute transform inline-block bg-white border p-2"
      :class="{ '-translate-y-full': thisposition === 'top', '-translate-x-full': thisposition === 'left' }">
      <slot name="content"></slot>
      <!--      那个三角形-->
      <div ref="sanjiaoxingref" class="absolute transform block bg-white border-t border-l w-2 h-2" :class="{
        '-top-1 left-1/2 -translate-x-1/2 rotate-45': thisposition === 'bottom',
        ...
        '-left-1 top-1/2 -translate-y-1/2 -rotate-45': thisposition === 'right'
      }"></div>
    </div>
    <!--    默认插槽-->
    <span class="inline-block" ref="triggerref" @click.stop="thisonClick()">
      <slot></slot>
    </span>
  </div>
</template>
